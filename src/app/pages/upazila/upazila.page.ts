...
import { getColorFromLabel, inputDebounceTime } from './../../utils/utility';
...
@Component({
  ...
  templateUrl: './upazila.page.html',
  styleUrls: ['./upazila.page.scss'],
})
export class UpazilaPage implements OnInit {

  periodStarting: string;
  periodEnding: string;
  pnriData: PnriData;
  identifierColumn = 'upazila';
  ...
  async ngOnInit() {
    ...
    if (val.ok) {
      ...
      this.currentScoreCards = this.pnriData.upazila_scorecards.scorecards;
      this.sortData();
  ...
  filterItems(searchTerm) {
    return this.pnriData.upazila_scorecards.scorecards.filter(item => item.upazila_name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1);
  }
